...
export default function Onboarding() {
  ...
  const techDomains = [
    { id: "software-dev", name: "Software Development", icon: Code, color: "bg-blue-500" },
    { id: "web-dev", name: "Web Development", icon: Code, color: "bg-cyan-500" },
  ...
  const techSkillsOptions = [
    ...
    "Kotlin", "TypeScript", "MongoDB", "PostgreSQL", "AWS", "Docker"
  ];

  const nonTechSkillsOptions = [
    "Video Editing", "Photography", "Content Writing", "Digital Marketing",
    "Project Management", "Public Speaking", "Creative Writing", "Graphic Design",
    "Social Media Management", "Event Planning", "Leadership", "Communication",
    "Research", "Presentation Skills", "Time Management", "Critical Thinking"
  ];
  ...
  const addUserProject = () => {
    setData(prev => ({
      ...prev,
      userProjects: [...prev.userProjects, { name: "", title: "", skillsUsed: [] }]
    }));
  };
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="text-center text-sm text-muted-foreground">
              Selected: {data.domains.length} domain{data.domains.length !== 1 ? 's' : ''}
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
                ...
                {techSkillsOptions.map((skill) => (
                  <Label
                    ...
                    className={`flex items-center space-x-2 cursor-pointer p-3 rounded-lg border text-sm transition-colors ${
                      data.skills.includes(skill) ? 'bg-primary/5 border-primary text-primary' : 'hover:bg-muted/50'
                    }`}
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label className="text-lg font-semibold">I have personal projects to showcase</Label>
              </div>
                    ...
                    <div key={index} className="space-y-3 p-4 border rounded-lg bg-background">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={project.name}
                          onChange={(e) => updateUserProject(index, 'name', e.target.value)}
                        />
                      </div>

                      <div>
                        <Label className="text-sm">Project Title/Description</Label>
                        <Input
                          placeholder="e.g., A responsive portfolio showcasing my work"
                          value={project.title}
                          onChange={(e) => updateUserProject(index, 'title', e.target.value)}
                        />
                      </div>

                      <div>
                        <Label className="text-sm">Skills Used (comma-separated)</Label>
                        <Input
                          placeholder="e.g., React, TypeScript, Tailwind CSS"
                          value={project.skillsUsed.join(', ')}
                          onChange={(e) => updateUserProject(index, 'skillsUsed', e.target.value.split(', ').filter(s => s.trim()))}
                        />
                      </div>
                    </div>
                  ))}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setData(prev => ({ ...prev, preferredPlatform: 'leetcode', gfgId: '', gfgRank: '' }));
                          }
                        }}
                      />
                      ...
                      <div className="ml-6 space-y-3 p-4 border rounded-lg bg-muted/30">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setData(prev => ({ ...prev, leetcodeId: e.target.value }))}
                          />
                        ...
                        <div>
                          <Label className="text-sm">LeetCode Rank (Optional)</Label>
                          <Input
                            placeholder="e.g., #12,345 or Guardian, Knight, etc."
                            value={data.leetcodeRank}
                            onChange={(e) => setData(prev => ({ ...prev, leetcodeRank: e.target.value }))}
                          />
                        </div>
                        {data.leetcodeId && (
                          <div className="mt-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setData(prev => ({ ...prev, preferredPlatform: 'gfg', leetcodeId: '', leetcodeRank: '' }));
                          }
                        }}
                      />
                      ...
                      <div className="ml-6 space-y-3 p-4 border rounded-lg bg-muted/30">
                        <div>
                          ...
                          <Input
                            placeholder="e.g., your_username"
                            value={data.gfgId}
                            onChange={(e) => setData(prev => ({ ...prev, gfgId: e.target.value }))}
                          />
                          <p className="text-xs text-muted-foreground mt-1">
                        ...
                        <div>
                          <Label className="text-sm">GFG Rank/Score (Optional)</Label>
                          <Input
                            placeholder="e.g., #5,678 or your score"
                            value={data.gfgRank}
                            onChange={(e) => setData(prev => ({ ...prev, gfgRank: e.target.value }))}
                          />
                        </div>
                        {data.gfgId && (
                          <div className="mt-2">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={data.githubId}
                      onChange={(e) => setData(prev => ({ ...prev, githubId: e.target.value }))}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground">Tell us what you want to achieve through our platform.</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {goalsOptions.map((goal) => (
                  <Label 
                    key={goal}
                    className={`flex items-center space-x-2 cursor-pointer p-3 rounded-lg border text-sm transition-colors ${
                      data.goals.includes(goal) ? 'bg-primary/5 border-primary text-primary' : 'hover:bg-muted/50'
                    }`}
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => setData(prev => ({ ...prev, customGoal: e.target.value }))}
                  rows={3}
                />
              </div>
            </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground">Help us understand how you discovered our platform.</p>
            </div>
            
            <RadioGroup value={data.heardFrom} onValueChange={(value) => setData(prev => ({ ...prev, heardFrom: value }))}>
              <div className="space-y-3">
                {heardFromOptions.map((option) => (
                  <Label key={option} className="flex items-center space-x-3 cursor-pointer p-4 rounded-lg border hover:bg-muted/50">
                    <RadioGroupItem value={option} />
                    <div className="flex items-center space-x-3">
                      {option.includes('Instagram') && <Instagram className="w-4 h-4 text-pink-500" />}
                      {option.includes('YouTube') && <Youtube className="w-4 h-4 text-red-500" />}
                      {option.includes('Twitter') && <Twitter className="w-4 h-4 text-blue-500" />}
            ...
            {data.heardFrom === 'other' && (
              <div className="mt-4">
                ...
                <Input
                  ...
                  onChange={(e) => setData(prev => ({ ...prev, customHeardFrom: e.target.value }))}
                />
              </div>
            )}
          </div>
  ...
  if (currentStep > totalSteps) {
    ...
    const getCompany = () => {
      const companies = ['Tech Startup', 'Growing Company', 'Innovation Lab', 'Digital Agency', 'Student'];
    ...
    const userProfile = {
      ...
      experience: data.experience === 'beginner' ? '< 1 Year' :
                  data.experience === 'intermediate' ? '1-3 Years' : '3+ Years',
      leetcodeRank: data.leetcodeRank || data.gfgRank || '--',
      domains: data.domains.map(domainId => {
        ...
        <div className="container mx-auto px-4 py-20">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl font-bold mb-4">Profile Card Created! 🎉</h1>
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Your profile will help us connect you with the right mentors and learning opportunities!
              </p>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-8">
              ...
              <div className="flex justify-between mt-8 pt-6 border-t">
                ...
                <Button 
                  ...
                  className="flex items-center space-x-2"
                >
                  <span>{currentStep === totalSteps ? 'Create Profile Card' : 'Next'}</span>
                  <ArrowRight className="w-4 h-4" />
                </Button>
              </div>
            </CardContent>
