...
interface CommunityStore {
  ...
  // Actions
  addCommunity: (
    community: Omit<Community, "id" | "createdAt" | "isJoined">,
  ) => void;
  addTrendingCommunity: (trendingData: any) => void;
  ...
  addProject: (
    communityId: string,
    project: Omit<CommunityProject, "id" | "createdAt">,
  ) => void;
  addHackathon: (communityId: string, hackathon: Omit<Hackathon, "id">) => void;
  addQuery: (
    communityId: string,
    query: Omit<Query, "id" | "timestamp">,
  ) => void;
...
const sampleCommunities: Community[] = [
  {
    ...
    description:
      "A community for React developers to share knowledge, collaborate on projects, and grow together.",
    category: "tech",
    privacy: "public",
    ...
    chatMessages: [
      {
        ...
        senderAvatar: "RK",
        content:
          "Hey everyone! Just finished implementing the new React 18 features in our project. Happy to share insights!",
    ...
    events: [
      {
        ...
        description:
          "Learn about React 18 new features and concurrent rendering",
        date: "2024-02-15",
        time: "18:00",
        type: "workshop",
        attendees: 45,
    ...
    projects: [
      {
        ...
        description:
          "Building a modern admin dashboard with React and TypeScript",
        author: {
          id: "1",
          name: "Rajesh Kumar",
          title: "Senior React Developer",
    ...
    queries: [
      {
        ...
        content:
          "I'm working on a large React application and noticing performance issues. What are the best practices for optimization?",
        author: {
          id: "3",
          name: "Amit Singh",
  ...
  },
  {
    ...
    description:
      "Explore the latest in AI and ML technologies, share research, and collaborate on cutting-edge projects.",
    category: "data",
    privacy: "public",
    rules: [
    ...
    (set, get) => ({
      ...
      addCommunity: (communityData) => {
        const newCommunity: Community = {
          ...
          chatMessages: [],
          pinnedMessages: [],
          domain: communityData.domain || "general",
          lastActivity: new Date().toISOString(),
        };
      ...
      // Helper method to add trending community to store
      addTrendingCommunity: (trendingData: any) => {
        const newCommunity: Community = {
          ...
          description:
            ...
            `A vibrant community for ${trendingData.name.toLowerCase()} enthusiasts`,
          category: "tech",
          privacy: "public",
          rules: ["Be respectful", "Stay on topic", "Help others"],
          tags: [trendingData.name.toLowerCase().replace(/[^a-z0-9]/g, "-")],
          members:
            parseInt(trendingData.members?.replace(/[^0-9]/g, "")) || 1000,
        ...
        set((state) => {
          ...
          const exists = state.communities.find(
            (c) => c.name === newCommunity.name,
          );
          if (exists) {
            ...
            const joinedCommunity = updatedCommunities.find(
              (c) => c.id === exists.id,
            );
            return {
        ...
        set((state) => ({
          communities: state.communities.map((community) =>
            community.id === communityId
              ? {
                  ...community,
                  hackathons: [newHackathon, ...community.hackathons],
                }
              : community,
          ),
        }));
                  ...
                  hackathons: community.hackathons.map((hackathon) =>
                    hackathon.id === hackathonId
                      ? {
                          ...
                          participants: hackathon.isParticipating
                  ...
                  projects: community.projects.map((project) =>
                    project.id === projectId
                      ? {
                          ...
                          interested: project.isInterested
                            ? project.interested - 1
                            : project.interested + 1,
                        }
                      : project,
                  ),
        ...
        set((state) => {
          ...
          if (exists) {
            ...
            return {
              ...
              joinedCommunities: joinedCommunity
                ? [
                    ...state.joinedCommunities.filter(
                      (c) => c.id !== exists.id,
                    ),
                    joinedCommunity,
                  ]
          ...
          communities: state.communities.map((community) =>
            community.id === communityId
              ? {
                  ...
                  pinnedMessages: community.pinnedMessages.filter(
                          ...
                          reactions: post.reactions
                            .map((reaction) =>
                              reaction.emoji === emoji
                                ? {
                                    ...reaction,
                                    count: reaction.users.includes(userId)
                                      ? reaction.count - 1
                                      : reaction.count + 1,
                                    users: reaction.users.includes(userId)
                                      ? reaction.users.filter(
